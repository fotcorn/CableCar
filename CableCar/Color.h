...
struct Color {
    ...
    Color(unsigned char r, unsigned char g, unsigned char b)
        : r(r), g(g), b(b), alpha(255) {}
    Color(std::string hexColor) {
        // strtoul also accepts strings starting with 0x or 0X, we do not want
        // that
        if (hexColor.size() >= 2 &&
            (hexColor.substr(0, 2) == std::string("0x") ||
             hexColor.substr(0, 2) == std::string("0X"))) {
            throw std::runtime_error(
                std::string(
                    "Color: Invalid html value, must not start with 0x or 0X") +
                hexColor);
        }
        ...
        try {
            ...
            throw std::runtime_error(
                std::string("Color: Invalid html value: ") + hexColor);
        }
        ...
        // with alpha value
            ...
            throw std::runtime_error(
                std::string(
                    "Color: Invalid html value, must be 6 or 8 chars long:") +
                hexColor);
        }
    }
