...
Color::Color(unsigned char r, unsigned char g, unsigned char b, unsigned char alpha) : r(r), g(g), b(b), alpha(alpha) {}
Color::Color(unsigned char r, unsigned char g, unsigned char b, float alpha) : r(r), g(g), b(b) {
    if (alpha < 0.0f || alpha > 1.0f) {
        throw std::runtime_error("Color: Invalid alpha value");
    }
    alpha = static_cast<unsigned char>(255.0 * alpha);
}
Color::Color(unsigned char r, unsigned char g, unsigned char b) : r(r), g(g), b(b), alpha(255) {}
Color::Color(std::string hexColor) {
    // strtoul also accepts strings starting with 0x or 0X, we do not want
    // that
    if (hexColor.size() >= 2 &&
        (hexColor.substr(0, 2) == std::string("0x") || hexColor.substr(0, 2) == std::string("0X"))) {
